-- ==========================================================
-- ADICIÓN DE NO-SHOWS DIARIOS (Supabase)
-- Proyecto: Nelux
-- Problema: Los no-shows no se ven en Trends porque faltan en metricas_diarias
-- ==========================================================

-- 1. Añadir columna a metricas_diarias si no existe
DO $$ 
BEGIN
    ALTER TABLE metricas_diarias ADD COLUMN IF NOT EXISTS no_shows INTEGER DEFAULT 0;
EXCEPTION WHEN others THEN 
    RAISE NOTICE 'La columna no_shows ya existe.';
END $$;

-- 2. Actualizar Función de Recálculo (SECURITY DEFINER ya aplicado en fix previo)
...
DECLARE
    ...
    v_no_shows INTEGER; -- Variable para el día
    v_mes TEXT;
    v_tx_prod INTEGER;
BEGIN
    ...
    SELECT 
        ...
        COUNT(*) FILTER (WHERE cancelada = true) -- CALCULAR NO-SHOWS DEL DÍA
    ...
    -- UPSERT MENSUAL (Ahora suma desde metricas_diarias para ser más eficiente)
    WITH mes_stats AS (
        SELECT 
