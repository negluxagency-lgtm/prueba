...
export default function StaffDashboard({ shopData, barber, onLogout }: StaffDashboardProps) {
    ...
    return (
        <div className="min-h-screen bg-black flex flex-col pt-12 md:pt-20 px-4 max-w-2xl mx-auto">
                                ...
                                <div className="flex flex-col gap-4 mb-6">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button onClick={() => { setEditingAppointment(null); setIsEditModalOpen(true); }} className="flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-1.5 md:py-2 bg-amber-500 text-black rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest hover:bg-amber-400 transition-all shadow-lg active:scale-95"><Plus className="w-3.5 h-3.5 md:w-4 h-4" /> <span>Nuevo</span></button>
            ...
            />
            <InvoiceModal isOpen={isInvoiceModalOpen} onClose={() => { setIsInvoiceModalOpen(false); setInvoiceAppointment(null); }} appointment={invoiceAppointment} shopData={{ id: shopData?.id, name: shopData?.nombre_barberia, address: shopData?.Direccion, phone: shopData?.telefono, email: shopData?.correo, cif: shopData?.['CIF/NIF'] }} />
