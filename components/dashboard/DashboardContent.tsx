...
export function DashboardContent({ userId, profile, fallback }: DashboardContentProps) {
    return (
        <SWRConfig value={{ fallback }}>
            <DashboardInner userId={userId} profile={profile} />
        </SWRConfig>
    );
}

function DashboardInner({ userId, profile }: { userId: string, profile: any }) {
    ...
    const {
        ...
        refreshAppointments
    } = useAppointments(selectedDate, userId);

    const { 
        shopData: { services, barbers }, 
        loading: shopLoading,
        refreshShopData
    } = useShopData(userId);
    ...
    const { chartData, loading: trendsLoading } = useTrends(userId, selectedDate);

    // --- CONSOLIDATED REALTIME ---
    useEffect(() => {
        if (!userId) return;
        // Canal principal consolidado para el dashboard
        const channel = supabase
            .channel(`dashboard-main-${userId}-${selectedDate}`)
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'citas', filter: `barberia_id=eq.${userId}` }, () => {
                refreshAppointments();
                mutateMetrics(); // Revalidar métricas también cuando hay cambios en citas
            })
    ...
    }, [userId, selectedDate, mutateMetrics, mutateSales, refreshAppointments, refreshShopData]);
    ...
    const isCadena = profile?.cadena === true;
    const cadenaIds = profile?.barberias_cadena_id ? (
        typeof profile.barberias_cadena_id === 'string' 
            ? JSON.parse(profile.barberias_cadena_id) 
            : profile.barberias_cadena_id
    ) : [];
    ...
    return (
        <main className="flex-1 p-4 lg:p-10 max-w-7xl mx-auto w-full pb-24 lg:pb-10">
                ...
                <DashboardStats 
                    ...
                    registerControl={
                        userId && (
                            <CashRegisterManager 
                                shopId={userId} 
                                userName={profile?.nombre_encargado}
            ...
            <InvoiceModal
                ...
                appointment={invoiceAppointment} shopData={profile ? { id: profile.id, name: profile.nombre_barberia || 'Mi Barbería', cif: profile["CIF/NIF"] || '' } : { id: '', name: 'Mi Barbería', cif: '' }}
