...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({ selectedDate, setSelectedDate }) => {
    return (
        <header className="mb-2 lg:mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-6">
            <div>
                <h1 className="text-3xl lg:text-5xl font-black italic tracking-tighter uppercase leading-tight">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-4 mt-4 md:mt-2">
                    ...
                    <div className="relative group w-fit mt-2 md:mt-0">
                        <input
                            ...
                            className="bg-zinc-900 border border-zinc-800 text-amber-500 text-[10px] md:text-sm font-bold px-2.5 py-1 md:px-4 md:py-2 rounded-lg md:rounded-xl focus:outline-none focus:border-amber-500 transition-all cursor-pointer appearance-none min-w-0 max-w-full [color-scheme:dark] [&::-webkit-calendar-picker-indicator]:opacity-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:w-full"
                        />
